...
for i in np.arange(0,len(panLaw)):
    ...
    vec=np.array([-X,-Y])
    vec=np.dot(np.power(vNorm,1/panLaw[i]["gamma"]),vec)
    L.append(vec)
    ...
    normSum=0
    for vec in L:
        normSum=normSum+np.power(np.linalg.norm(vec),2/panLaw[i]["gamma"])
    V=np.dot(np.add.reduce(L),1/normSum)
    ...
    sumSquareVec=[]
    for nVec in L:
        sumSquareVec.append(np.dot(np.linalg.norm(nVec),nVec))
        sumSquare=sumSquare+np.power(np.linalg.norm(nVec),2)
