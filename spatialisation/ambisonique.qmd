...
w_r = 1
w_x = w_r*np.outer(np.cos(phi), np.sin(theta))
w_y = w_r*np.outer(np.sin(phi), np.sin(theta))
w_z = w_r*np.outer(np.ones(np.size(phi)), np.cos(theta))

x_x = np.outer(np.cos(phi)*np.cos(phi), np.sin(theta)*np.sin(theta))
x_y = np.outer(np.cos(phi)*np.sin(phi), np.sin(theta)*np.sin(theta))
x_z = np.outer(np.cos(phi)*np.ones(np.size(phi)), np.sin(theta)*np.cos(theta))

# Plot the surface
# ax.plot_surface(w_x, w_y, w_z)
ax.plot_surface(x_x, x_y, x_z)
