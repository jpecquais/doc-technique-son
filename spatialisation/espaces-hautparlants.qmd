...
#| fig-align: "center"
#| fig-cap: "Contribution des haut-parleurs et leur résultante, en rouge, pour une source virtuelle, spatialisée en VBAP"
...
X=speakers[0][0]
Y=speakers[0][1]
vNorm=(np.sqrt(np.sqrt(1+q(sourceAzim))/(np.sqrt(1+q(sourceAzim))+np.sqrt(1-q(sourceAzim)))))
...
+ Et en bleu, le vecteur résultante normalisé $\vec{E}$ du carré des vecteurs de norme $r_E$, indiquant l'énergie du système
...
$\vec{V}$, appelé vecteur vélocité, s'apparente à la propagation en champ libre de la somme des ondes acoustiques produites par les haut-parleurs. Le vecteur énergie $\vec{E}$ indique, ici, la direction moyenne de la provenance de l'énergie. On remarque que sa norme est maximale (égale à un) lorsqu'un seul et unique haut-parleur contribue à la diffusion du son.
