...
for i in np.arange(0,len(sourceAzim)):
    ...
    X=speakers[0][0]
    Y=speakers[0][1]
    vNorm=(1+q(sourceAzim[i]))/(np.sqrt(np.power(1+q(sourceAzim[i]),2)+np.power(1-q(sourceAzim[i]),2)))
...
X=speakers[0][0]
Y=speakers[0][1]
vNorm=(1+q(sourceAzim))/(np.sqrt(np.power(1+q(sourceAzim),2)+np.power(1-q(sourceAzim),2)))
