...
## Evaluation objective des espaces haut-parlants

Lorsque l'on étudie un espace haut-parlant, on cherche rapidement à évaluer la précision de localisation dont il est capables. Pour cela, on étudie la contribution de chaque haut-parleur du système pour une source sonore à reproduire donnée (et donc pour une technique de spatialisation précise). Il est dans ce cas commode d'utiliser une approche vectorielle, où, pour chaque enceinte, on lui attribue un vecteur, ayant pour norme son gain et poitant vers l'auditeur.
...
q = lambda phi: np.tan(phi)/np.tan(np.pi/4)

theta = -np.pi/8
alphaSource = [1.4, 1.4*np.sin(theta)/np.cos(theta)]
speakers = [[1,1], [-1,1], [-1,-1], [1,-1]]
# speakerWeight = [1.3, 1.3, 1.3, 1.3] #in dB
spkrGain = [0.3, 0.1, 0, 0.6]

fig, ax = plt.subplots()
fig.tight_layout()

img = np.asarray(Image.open('../_resources/bitmap/various/head.png'))
ax.imshow(img)
for speakerPos in speakers:
    ax.plot(speakerPos[0]*1.05, speakerPos[1]*1.05, "D", ms=20, color='black')
ax.plot(alphaSource[0], alphaSource[1], "o", ms=20, color='purple')
speakers = [[1,1], [-1,1], [-1,-1], [1,-1]]
...
# Directional vectors
U = (np.sqrt(np.sqrt(1+q(theta))/(np.sqrt(1+q(theta))+np.sqrt(1-q(theta)))))*-1/np.sqrt(2)  
V = (np.sqrt(np.sqrt(1+q(theta))/(np.sqrt(1+q(theta))+np.sqrt(1-q(theta)))))*-1/np.sqrt(2)  
L.append([U,V])

ax.quiver(X, Y, U, V, color='b', units='xy', scale=1)
...
# Directional vectors
U = 0.583*-np.cos(-np.pi/8)  
V = 0.583*-np.sin(-np.pi/8)

# L=np.sum([vec1,vec2])
L=np.add.reduce(L)
ax.quiver(0, 0, -L[0], -L[1], color='r', units='xy', scale=1)
...
En bleu, les vecteurs de la contribution de chacun des haut-parleurs, en rouge, le vecteur résultant $\vec{V}$ de norme $R_v$ 
...
![Monophonie](../_resources/bitmap/speaker/mono_label.png)

![Stereophonie](../_resources/bitmap/speaker/stereo_label.png)
...
![Quadriphonie](../_resources/bitmap/speaker/quad_label.png)

![Surround](../_resources/bitmap/speaker/lcrs_label.png)
...
![5.1](../_resources/bitmap/speaker/5_1_label.png)

![7.1](../_resources/bitmap/speaker/7_1_label.png)
...
![Octophonie](../_resources/bitmap/speaker/8_0_label.png)
