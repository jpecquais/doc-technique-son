...
Soit le schéma électronique ci-dessous. On appelle \(U_{out}\) la tension de sortie de l'appareil A et \(Z_{out}\) son impédance de sortie. De façon similaire, on appelle \$U\_\{in\} la tension d'entré de l'appareil B et \(Z_{in}\) son impédance d'entré.
...
Dès lors, si \(Z_{in}\) est très grand devant \(Z_{out}\), alors \(U_{in}\) tend vers \(U_{out}\). Si \(Z_{out}\) est très grand devant \$Z\_\{in\}, alors \(U_{in}\) tend vers \(0\).
